...
[rFrustMat,cFrustMat] = find(G.adjMat);
vFrustMat = zeros(size(rFrustMat));
for j=1:length(vFrustMat)
    vFrustMat(j) = perEdgeFrustMat(rFrustMat(j),cFrustMat(j));
end
vFrustMat_rescaled = (vFrustMat-min(vFrustMat))/(max(vFrustMat)-min(vFrustMat)+eps);
